import { useEffect, useState } from "react";
import { Helmet } from "react-helmet";
import {
  Link,
  useHistory,
  useParams,
} from "react-router-dom/cjs/react-router-dom.min";
import Grid from "../../../components/Grid";
import { upcomingMovieUrl } from "../../../utility/util";

const UpcomingMovie = () => {
  const [upcomingMovie, setUpcomingMovie] = useState(null);
  const [isPending, setIsPending] = useState(true);
  const { page } = useParams();
  const history = useHistory();

  useEffect(() => {
    setIsPending(true);
    fetch(upcomingMovieUrl(page))
      .then((res) => res.json())
      .then((data) => {
        setIsPending(false);
        setUpcomingMovie(data.results);
      });
  }, [page]);

  return (
    <section>
      <Helmet>
        <title>Pardiwan Movie Updates | Upcoming</title>
      </Helmet>

      <h2 className="w-full text-center text-white text-lg font-bold bg-primaryLight rounded-xl py-2 ring-1 ring-gray-800">
        Upcoming Movie
      </h2>
      <div className=" bg-primaryLight rounded-xl p-2 ring-1 ring-gray-800 mt-4">
        <div className="flex justify-between items-center">
          <h4 className="text-white">Page {page}</h4>
          {page != 1 && (
            <Link
              to={`/movie/upcoming/${parseInt(page) - 1}`}
              className="text-white hover:text-blue-500 ml-auto mr-2"
            >
              &#9665;
            </Link>
          )}
          <Link
            to={`/movie/upcoming/${parseInt(page) + 1}`}
            className="text-white hover:text-blue-500"
          >
            &#9655;
          </Link>
        </div>
        <div>
          {isPending && (
            <p className="block w-full px-8 py-12 text-white font-bold text-xl text-center">
              Loading...
            </p>
          )}
          {upcomingMovie && !isPending && (
            <Grid
              data={upcomingMovie}
              clicked={(id) => history.push(`/movie/${id}`)}
            ></Grid>
          )}
        </div>
      </div>
    </section>
  );
};

export default UpcomingMovie;
